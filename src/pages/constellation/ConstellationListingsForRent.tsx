...
export default function ConstellationListingsForRent() {
  ...
  if (loading) {
    return (
      <>
        <Helmet>
          <title>{isPt ? 'Imóveis para Alugar' : 'For Rent Listings'} - Constellation</title>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 pt-32 pb-16 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-white">
                    {isPt ? 'Imóveis para Alugar' : 'For Rent Listings'}
                  </h1>
            ...
            <Link
              to="/dashboard/listings/create?type=for_rent"
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500/50"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white focus:outline-none focus:border-indigo-500/50"
            >
              ...
              <option value="rented">{isPt ? 'Alugados' : 'Rented'}</option>
          ...
          {filteredListings.length === 0 ? (
            <div className="text-center py-16 bg-slate-800/30 rounded-2xl border border-slate-700/50">
              ...
              <h3 className="text-lg font-medium text-white mb-2">
                {isPt ? 'Nenhum imóvel para alugar' : 'No rental listings'}
              </h3>
              <p className="text-slate-400 mb-6">
                {isPt ? 'Adicione seu primeiro imóvel para aluguel' : 'Add your first rental listing'}
              </p>
              <Link
                to="/dashboard/listings/create?type=for_rent"
                className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-lg font-medium transition-all"
              >
                <Plus className="w-4 h-4" />
                  ...
                  <div className="relative aspect-[4/3] bg-slate-900">
                    {listing.media?.[0]?.url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Key className="w-12 h-12 text-slate-700" />
                      </div>
                    )}
                      ...
                      <div className="relative">
                        ...
                        {activeMenu === listing.id && (
                          <div className="absolute right-0 bottom-full mb-1 w-40 bg-slate-800 border border-slate-700 rounded-lg shadow-xl py-1 z-10">
                            ...
                            <button
                              onClick={() => updateStatus(listing.id, 'rented')}
                              className="flex items-center gap-2 px-3 py-2 text-sm text-purple-400 hover:bg-slate-700/50 w-full"
                            >
                              <Key className="w-4 h-4" />
                              {isPt ? 'Marcar Alugado' : 'Mark Rented'}
                            </button>
                            <button
