...
export default function ConstellationListingsForSale() {
  ...
  if (loading) {
    return (
      <>
        <Helmet>
          <title>{isPt ? 'Imóveis à Venda' : 'For Sale Listings'} - Constellation</title>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-white">
                    {isPt ? 'Imóveis à Venda' : 'For Sale Listings'}
                  </h1>
                  <p className="text-sm text-slate-400">
                    {listings.length} {isPt ? 'imóveis' : 'listings'}
                  </p>
                </div>
              </div>
            </div>
            <Link
              to="/dashboard/listings/create?type=for_sale"
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white rounded-lg font-medium transition-all shadow-lg hover:shadow-xl"
            >
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder={isPt ? 'Buscar imóveis...' : 'Search listings...'}
                className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500/50"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white focus:outline-none focus:border-emerald-500/50"
            >
              ...
              <option value="sold">{isPt ? 'Vendidos' : 'Sold'}</option>
          ...
          {filteredListings.length === 0 ? (
            <div className="text-center py-16 bg-slate-800/30 rounded-2xl border border-slate-700/50">
              ...
              <h3 className="text-lg font-medium text-white mb-2">
                {isPt ? 'Nenhum imóvel à venda' : 'No sale listings'}
              </h3>
              <p className="text-slate-400 mb-6">
                {isPt ? 'Adicione seu primeiro imóvel para venda' : 'Add your first sale listing'}
              </p>
              <Link
                to="/dashboard/listings/create?type=for_sale"
                className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white rounded-lg font-medium transition-all"
              >
                <Plus className="w-4 h-4" />
                {isPt ? 'Criar Anúncio' : 'Create Listing'}
              </Link>
            </div>
          ) : (
                        ...
                        {activeMenu === listing.id && (
                          <div className="absolute right-0 bottom-full mb-1 w-40 bg-slate-800 border border-slate-700 rounded-lg shadow-xl py-1 z-10">
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 px-3 py-2 text-sm text-blue-400 hover:bg-slate-700/50 w-full"
                            >
                              <CheckCircle className="w-4 h-4" />
                              {isPt ? 'Marcar Vendido' : 'Mark Sold'}
                            </button>
                            <button
