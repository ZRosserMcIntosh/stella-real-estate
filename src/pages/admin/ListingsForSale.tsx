...
export default function ListingsForSale() {
	...
	async function saveEdit() {
		...
		<div>
			<h1 className="text-xl font-semibold text-slate-100">Listings · For Sale</h1>
			<p className="mt-1 text-slate-400">Create and manage properties listed for sale.</p>
			{isDemo && (
			...
			{error && <p className="text-sm text-red-500 mt-1">{error}</p>}
				...
				<div className="mt-6 grid grid-cols-1 md:grid-cols-[260px,1fr] gap-4 items-start">
					...
					<aside className="rounded-xl border border-slate-700/60 bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-lg p-4 sticky top-4 h-fit shadow-lg shadow-slate-950/20">
					...
					{filtered.map(l => {
						...
						return (
							<div key={l.id} className="rounded-xl border border-slate-700/60 bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-lg p-3">
								{thumb ? (
								...
								<div className="mt-2 flex items-start justify-between gap-3">
									<div>
										<h3 className="font-semibold line-clamp-1 text-slate-100">{l.title}</h3>
										<p className="text-sm text-slate-400">{[l.city, l.state_code].filter(Boolean).join(', ')}</p>
										<p className="text-sm text-slate-300 mt-1">{l.price ? `R$ ${Number(l.price).toLocaleString()}` : '—'}</p>
									</div>
									<div className="flex items-center gap-3">
						<button onClick={()=>startEdit(l)} className="text-sm text-indigo-400 hover:text-indigo-300 hover:underline">Edit</button>
						<button
							onClick={()=>remove(l.id)}
							disabled={isDemo}
				...
				{editingId && (
					<div className="fixed inset-0 bg-black/50 backdrop-blur-sm grid place-items-center z-50">
						<div className="w-full max-w-2xl rounded-xl border border-slate-700/60 bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-lg p-4 text-slate-100 shadow-2xl shadow-slate-950/50">
