...
export function CreateTaskModal({ project, users, currentUser, existingTasks, onClose, onCreate }: CreateTaskModalProps) {
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="rounded-lg border border-slate-200/70 p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-semibold text-slate-700">Milestone</h4>
                    <p className="text-xs text-slate-500">Highlight this task on timeline and calendar.</p>
                  </div>
                  <button
                    type="button"
                    ...
                    className={`rounded-full px-3 py-1 text-xs font-semibold ${
                      formData.isMilestone
                        ? 'bg-pink-600/20 text-pink-600 border border-pink-500/40'
              ...
              <div>
                ...
                <div className="max-h-32 overflow-y-auto rounded-lg border border-slate-200">
                  {dependencyOptions.length === 0 ? (
                    <div className="p-3 text-xs text-slate-500">No other tasks yet.</div>
                  ) : (
                    dependencyOptions.map(task => (
                      <label
                        key={task.id}
                        className="flex cursor-pointer items-center gap-2 border-b border-slate-100/60 px-3 py-2 text-xs last:border-b-0 hover:bg-slate-50"
                      >
                        <input
                          type="checkbox"
                          checked={formData.dependsOn.includes(task.id)}
                          onChange={() => toggleDependency(task.id)}
                          className="rounded border-slate-400 text-pink-500 focus:ring-pink-500/40"
                        />
