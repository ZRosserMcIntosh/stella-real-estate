...
export default function ListingsForRent() {
	...
	const createListing = async (e: React.FormEvent) => {
		...
		try {
			...
			const insert = {
				listing_type: 'for_rent' as const,
				title,
				...
				features: {
					...
					...(minLeaseTerm !== '' ? { min_lease_months: Number(minLeaseTerm) } : {}),
				},
		...
		<div>
			...
			<div className="mt-6 rounded-xl border border-slate-700/60 bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-lg">
				<button type="button" onClick={()=>setShowForm(s=>!s)} className="w-full flex items-center justify-between px-4 py-3">
					<span className="font-medium text-slate-100">Add a new listing</span>
					<svg className={"h-5 w-5 text-slate-400 transition-transform " + (showForm ? "rotate-180" : "rotate-0")} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
				...
				{showForm && (
					<div className="border-t border-slate-700/60 p-4">
						<form onSubmit={createListing} className="grid gap-3">
							<label className="grid gap-1 text-sm">
								...
								<input className={inputCls} value={title} onChange={e=>setTitle(e.target.value)} required placeholder="e.g., 2BR Apartment for rent in Copacabana" />
							</label>
							<label className="grid gap-1 text-sm">
							...
							{/* Currency row: Price, Condominium fee, IPTU, Minimum lease term */}
							<div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
								<label className="grid gap-1 text-sm">
									<span className="font-medium">Price</span>
									<input type="text" inputMode="numeric" className={inputCls} value={price} onChange={e=>setPrice(formatBRLInput(e.target.value))} placeholder={toBRL(0)} />
								</label>
								<label className="grid gap-1 text-sm">
								...
								<label className="grid gap-1 text-sm">
									...
									<input type="number" min={1} step="1" className={inputCls} value={minLeaseTerm} onChange={e=>setMinLeaseTerm(e.target.value === '' ? '' : Number(e.target.value))} placeholder="e.g., 12" />
								</label>
							</div>
					...
					{FEATURES.property.map(key => (
						...
						<span className="capitalize">{featureLabel(key)}</span>
									...
									{/* Churrasqueira + Stove + Internet + Custom */}
			...
			<div className="mt-6 grid grid-cols-1 sm:grid-cols-[260px,1fr] gap-4 items-start">
				...
				<aside className="rounded-xl border border-slate-700/60 bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-lg p-4 sticky top-4 h-fit">
					<h2 className="text-sm font-semibold mb-2 text-slate-100">Filters</h2>
					<div className="grid gap-2 text-sm">
						...
						<label className="grid gap-1">
							...
							<select value={fStatus} onChange={e=>setFStatus(e.target.value as any)} className={selectCls}>
								<option value="all">Any</option>
								{['draft','active','pending','rented','archived'].map(s => <option key={s} value={s}>{s}</option>)}
							</select>
						</label>
					</div>
				</aside>
				...
				{filtered.map(l => {
					...
					return (
						<div key={l.id} className="rounded-xl border border-slate-700/60 bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-lg p-3 text-slate-300">
							{thumb ? (
								<img src={thumb} alt={l.title} className="w-full h-40 object-cover rounded-md" />
							) : (
								<div className="w-full h-40 bg-slate-800/50 rounded-md grid place-items-center text-slate-500 text-sm">No image</div>
							)}
							<div className="mt-2 flex items-start justify-between gap-3">
								<div>
									<h3 className="font-semibold line-clamp-1 text-slate-200">{l.title}</h3>
									<p className="text-sm text-slate-400">{[l.city, l.state_code].filter(Boolean).join(', ')}</p>
									<p className="text-sm text-slate-300 mt-1">{l.price != null ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(l.price)) : '—'}</p>
								</div>
			...
			{editingId && (
				<div className="fixed inset-0 bg-black/50 backdrop-blur-sm grid place-items-center z-50">
					<div className="w-full max-w-2xl rounded-xl border border-slate-700/60 bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-lg p-4 text-slate-300 shadow-2xl shadow-slate-950/50">
						<h3 className="text-lg font-semibold text-slate-100">Edit listing</h3>
